          ...
          <div ref={panelRef} className="lg:flex-2">
                ...
                <pre className="bg-card border-border text-muted hover:border-accent/30 overflow-x-auto rounded-lg border p-5 font-mono text-sm transition duration-400">
                  <code>
                    ...
                    {entries.map(({ label, value }) => (
                      <span key={label} className="text-foreground block pl-6">
                        {label}:{" "}
                        {Array.isArray(value)
                          ? `[${value.map((v) => `'${v}'`).join(", ")}]`
                          : `'${value}'`}
            ...
            {activeTab === "skills" && (
              <div
                ...
                className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 lg:gap-8"
              >
                <SkillCard name="Java">
                ...
                <SkillCard name="Angular">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" className="size-7">
                    <path
                      fill="currentColor"
